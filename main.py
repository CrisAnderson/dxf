...
lapproach = 845 #lower approach of the exyz crane
uapproach = 2045 #upper approach min
...
for l in range(aisles):
    ...
    for k in range(bays):
        ...
        # create first bay
        for i in range(bay):
            blockref = dxf.insert(blockname='pallet', insert=((i*(loadx+palgap)+upx+upgap)+blength,(aislew/2)+aisle2,lapproach))
            dwg.add(blockref)
        # create opposite side
        for j in range(bay):
            blockref = dxf.insert(blockname='pallet', insert=((j*(loadx+palgap)+upx+upgap)+blength,0-(aislew/2)-loady+aisle2,lapproach))
            dwg.add(blockref)
